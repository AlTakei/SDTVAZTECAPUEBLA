...
class TicketsController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            // Aplicar el filtro por estado antes de construir la tabla DataTables
            if ($request->has('status_id')) {
                $statusId = $request->get('status_id');
                $query->where(function($query) use ($statusId) {
                    $query->where('status_id', $statusId)
                        ->orWhereHas('status', function($query) use ($statusId) {
                            $query->where('name', Status::findOrFail($statusId)->name);
                        });
                });
            }
        ...
        // Obtener el número de tickets cerrados
        $closedTickets = Ticket::where(function ($query) {
            $query->where('status_id', 2) // Buscar por ID del estado en inglés
                ->orWhereHas('status', function ($query) {
                    $query->where('name', 'Cerrado'); // Buscar por nombre del estado en español
                });
        })->count();
        ...
        return view('admin.tickets.index', compact('priorities', 'statuses', 'categories', 'tickets', 'totalTickets', 'openTickets', 'closedTickets'));
    }
